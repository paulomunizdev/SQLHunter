...
/*
 * @brief                  Function to write data received from curl into a string buffer.
...
/*
 * @brief                  Function to check if the server response contains SQL syntax error.
 * @param response         The response received from the server.
 * @return bool            True if SQL syntax error is found, otherwise false.
 */
bool
hasSQLError (const std::string & response)
{
  // Common patterns for SQL syntax error
  std::string patterns[] =
	{ "SQL syntax", "SQL Error", "MySQL Error", "syntax error" };
...
/*
 * @brief                  Function to check if the URL is susceptible to SQL injection.
 * @param url              The URL to be checked.
 * @return bool            True if URL is vulnerable, otherwise false.
 */
		 bool isSQLInjectionVulnerable (const std::string & url)
		 {
		 std::cout << "Processing URL: " << url << std::endl;	// Print the processed URL
		 // Basic CURL setup
		 CURL * curl = curl_easy_init ();
		 if (!curl)
		 {
		 std::cerr << "Error initializing CURL." << std::endl; return false;}
		 ...
		 curl_easy_setopt (curl, CURLOPT_URL, test_url.c_str ()); curl_easy_setopt (curl, CURLOPT_FOLLOWLOCATION, 1L);	// Follow redirects
		 curl_easy_setopt (curl, CURLOPT_WRITEFUNCTION, writeFunction);	// Function for writing data received from curl
		 ...
		 if (res != CURLE_OK)
		 {
		 std::cerr << "Error executing CURL request: " <<
		 curl_easy_strerror (res) << std::endl; curl_easy_cleanup (curl);
		 return false;}

		 // Check if the response indicates an SQL syntax error
		 bool is_vulnerable = hasSQLError (response_buffer);
		 // Clean up CURL
		 curl_easy_cleanup (curl); return is_vulnerable;}
 ...
 */
		 ...
		 {
		 std::string decoded;
		 size_t length = url.length (); for (size_t i = 0; i < length; ++i)
		 {
 ...
 */
		 void searchAndPrintLinks (const std::string & html,
		 ...
		 std::string link = match.str (1); if (link.find ("/url?q=") == 0)
		 {
		 size_t startPos = link.find ("=") + 1;
		 size_t endPos = link.find ("&"); if (endPos != std::string::npos)
		 {
		 link = link.substr (startPos, endPos - startPos);}
		 if (link.find ("google.com") == std::string::npos)
		 {
		 CURLU * url = curl_url (); if (url)
		 {
		 ...
		 curl_free (urlDecoded); curl_url_cleanup (url);}
		 }
		 }
		 ++iter;}
		 }
 ...
 */
		 ...
		 std::
		 cout <<
		 "Before proceeding, please ensure you are using a proxy if necessary.\n"
		 << std::endl; int choice;
...
case 1:
		 {
		 // Dork Scanner option
		 std::cout << "Please put the dorks in the 'dorks.txt' file. Press Enter when ready." << std::endl; std::cin.ignore ();	// Clear input buffer
		 std::cin.get ();		// Wait for user to press Enter
		 std::cout << "Starting the Dork Scanner..." << std::endl;
		 // Open "dorks.txt" file for reading
		 std::ifstream dorksFile ("dorks.txt"); if (!dorksFile.is_open ())
		 {
		 std::cerr << "Error opening the 'dorks.txt' file." << std::endl;
		 return 1;}

		 // Open "links.txt" file for writing
		 std::ofstream linksFile ("links.txt", std::ios_base::trunc);	// Overwrite previous content
		 ...
		 std::string dork; while (std::getline (dorksFile, dork))
		 {
		 dorks.push_back (dork);}
		 dorksFile.close (); int numPages;
		 // Prompt user to enter the number of pages to scan
		 std::cout << "Enter the number of pages to scan: ";
		 std::cin >> numPages;
		 std::string base_search_query;
		 std::string search_query; std::string response_buffer;
		 // Create the "links.txt" file with edit permissions
	chmod ("links.txt", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH); for (const auto & dork:dorks)
		 {
		 std::string modifiedDork = dork;
		 base_search_query =
		 "https://www.google.com/search?q=" + modifiedDork; for (int page = 0;
																 page <
																 numPages;
																 ++page)
		 {
		 ...
		 {
		 std::cerr << "Error initializing cURL." << std::endl; return 1;}
		 res = performRequest (curl, search_query, response_buffer);
		 if (res != CURLE_OK)
		 {
		 std::cerr << "Failed to execute HTTP request." << std::endl;
		 linksFile.close (); curl_easy_cleanup (curl); return 1;}
		 std::cout << "Links on page " << page +
		 1 << " for the dork \"" << dork << "\":" << std::endl;
...
case 2:
		 {
		 // Vuln Scanner option
		 std::cout << "Please put the links in the 'links.txt' file. Press Enter when ready." << std::endl; std::cin.ignore ();	// Clear input buffer
		 std::cin.get ();		// Wait for user to press Enter
		 std::cout << "Starting the Vuln Scanner..." << std::endl;
		 // Open "links.txt" file for reading
		 std::ifstream linksFile ("links.txt"); if (!linksFile.is_open ())
		 {
		 std::cerr << "Error opening the 'links.txt' file." << std::endl;
		 return 1;}

		 // Open "vuln.txt" file for writing
		 std::ofstream vulnFile ("vuln.txt", std::ios_base::trunc);	// Overwrite previous content
		 if (!vulnFile.is_open ())
		 {
...
// Option 3: Dork Scanner followed by Vuln Scanner
		 std::cout <<
		 "Option 3 selected: Dork Scanner followed by Vuln Scanner." << std::
		 endl;
// Open "dorks.txt" file for reading
		 std::ifstream dorksFile ("dorks.txt"); if (!dorksFile.is_open ())
		 {
...
// Change permissions of the "links.txt" file to allow editing
		 ...
		 std::vector < std::string > dorks; std::string dork;
...
// Prompt user to enter the number of pages to scan
	...
	std::string base_search_query; std::string search_query; std::string response_buffer; for (const auto & dork:dorks)
		 {
		 std::string modifiedDork = dork;
		 ...
		 curl_easy_cleanup (curl);
		 std::cout <<
		 "Dork Scanner process completed. Links saved to the 'links.txt' file."
		 << std::endl;
// Execute Vuln Scanner
		 std::cout << "Starting the Vuln Scanner after the Dork Scanner..." <<
		 std::endl;
// Open "links.txt" file for reading
		 std::ifstream file ("links.txt"); if (!file.is_open ())
		 {
		 std::cerr << "Error opening the 'links.txt' file." << std::endl;
		 return 1;}
...
// Read URLs from the file until the end
		 std::string url; while (std::getline (file, url))
		 {
		 // Check if the URL is vulnerable
		 if (isSQLInjectionVulnerable (url))
		 {
...
// Close files
		 ...
		 vulnFile.close ();
		 std::cout <<
		 "Vuln Scanner process completed. Vulnerable URLs have been added to the 'vuln.txt' file."
		 << std::endl; break;}
default:
		 std::
		 cerr << "Invalid option. Please choose a valid option." << std::endl;
		 break;}
