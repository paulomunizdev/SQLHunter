 ...
 */
bool
isSQLInjectionVulnerable (const std::string &url)
{
      ...
      if (url[i] == '%' && i + 2 < length && isxdigit (url[i + 1])
          && isxdigit (url[i + 2]))
        {
          decoded += static_cast<char> (
              std::stoi (url.substr (i + 1, 2), nullptr, 16));
          i += 2;
        }
      else
        {
 ...
 */
void
searchAndPrintLinks (const std::string &html, std::ofstream &outputFile)
{
              ...
              if (url)
                {
                  ...
                  char *urlDecoded;
                  if (curl_url_get (url, CURLUPART_URL, &urlDecoded, 0)
                      == CURLUE_OK)
                    {
 ...
 */
CURLcode
performRequest (CURL *curl, const std::string &search_query,
                std::string &response_buffer)
{
  CURLcode res;
...
main ()
{
  ...
  switch (choice)
    ...
    case 1:
      {
        // Dork Scanner option
        std::cout << "Please put the dorks in the 'dorks.txt' file. Press "
                     "Enter when ready."
                  << std::endl;
        ...
        for (const auto &dork : dorks)
          {
            ...
            base_search_query
                = "https://www.google.com/search?q=" + modifiedDork;
    ...
    case 2:
      {
        // Vuln Scanner option
        std::cout << "Please put the links in the 'links.txt' file. Press "
                     "Enter when ready."
                  << std::endl;
        std::cin.ignore (); // Clear input buffer
        std::cin.get ();    // Wait for user to press Enter
    ...
    case 3:
      {
        ...
        file.close ();
        vulnFile.close ();
        std::cout << "Vuln Scanner process completed. Vulnerable URLs have "
                     "been added to the 'vuln.txt' file."
                  << std::endl;
        break;
      }
    default:
